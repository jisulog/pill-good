...
function ManagerLecUpdateContainer(props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {...register("title", {minLength: {value: 3, message: "3자 이상 입력하세요!" }, maxLength: {value: 20, message: "20자 이하 입력하세요!" }})}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)} placeholder="3~20자" /><br />
        {errors.title && <p role="alert">{errors.title.message}</p>}

        날짜: <input type="date" id="date" name="date" value={lec.date||""} 
        {...register("date", {validate:{
          dateTrue : (value)=> moment(lec.date).format("YY.MM.DD") > today
        }
      })}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)} /><br />
      {errors.date && errors.date.type !== "dateTrue" && (<p>잘못 입력하셨습니다 :(</p>)}

        시간: <input type="time" id="time" name="time"  value={lec.time||""} {...register("time")}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)}/><br />
        ...
        인원:
        <select name="number" id="number" value={lec.number||""} {...register("number")}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)}>
          <option value="">--인원--</option>
          <option value="1">1인</option>
          <option value="2">2인</option>
        ...
        난이도: 
        <select name="level" id="level" value={lec.level||""} {...register("level")}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)}>
          <option value="">--난이도--</option>
          <option value="1">level-1</option>
          <option value="2">level-2</option>
        ...
        <textarea name="content" id="content" value={lec.content||""} {...register("content")}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)}></textarea><br />
        {errors.content && <p>입력하세요!</p>}
        
        <div name="status" id="status" value={lec.status} {...register("status")}
        onChange={(e)=>setLecProps(e.target.name, e.target.value)}>
          <input type="radio" id="status2" name="status" value="2" />
          <label htmlFor="o">승인</label>
          <input type="radio" id="status3" name="status" value="3" />
